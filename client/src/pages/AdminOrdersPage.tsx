    ...
    <div className="flex h-full flex-col gap-5 font-sans">
      ...
      <div className="flex items-end justify-between px-0.5">
        ...
        <div className="flex items-center gap-2.5">
          <button
            onClick={loadOrders}
            className="rounded-xl border border-[#D1D5DB] bg-white px-4 py-2.5 text-[13px] font-semibold text-dark transition-colors hover:bg-gray-50"
          >
            Оновити
          </button>
          <button className="rounded-xl bg-[#F59E0B] px-4 py-2.5 text-[13px] font-bold text-dark transition-opacity hover:opacity-90">
            Експорт CSV
          </button>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1 gap-5">
        ...
        <div className="flex flex-1 flex-col overflow-hidden rounded-2xl border border-[#DDE3EA] bg-white">
          {/* Header row */}
          <div className="flex gap-2.5 border-b border-[#E5E7EB] bg-[#F9FAFB] px-3.5 py-3">
            ...
            {loading ? (
              ...
              filtered.map((order) => (
                <div
                  ...
                  className={`flex cursor-pointer items-center gap-2.5 border-b border-[#F1F5F9] px-3.5 py-3 transition-colors hover:bg-[#F8FAFC] ${
                    selected?.id === order.id ? "bg-[#FFFBEB]" : ""
                  }`}
                >
                  <span className="w-[200px] shrink-0 truncate text-[13px] font-semibold text-dark">
                    {order.customerName}
                  </span>
                  <span className="w-[140px] shrink-0 text-[13px] font-medium text-dark">
                    {order.phone}
                  </span>
                  <span className="w-[120px] shrink-0 truncate text-[13px] font-medium text-dark">
                    {order.equipment.name}
                  </span>
                  <span className="w-[100px] shrink-0 text-[13px] font-medium text-dark">
                    {new Date(order.createdAt).toLocaleDateString("uk")}
                  </span>
                  <span
                    className={`w-[90px] shrink-0 text-[13px] font-bold ${statusTextColors[order.status] ?? "text-dark"}`}
                  >
        ...
        {/* Right — detail panel */}
        <div className="flex w-[380px] shrink-0 flex-col gap-3.5 overflow-y-auto rounded-2xl border border-[#DDE3EA] bg-[#FCFCFD] p-[18px]">
          {selected ? (
            <>
              ...
              <DetailField label="Техніка" value={selected.equipment.name} />
            ...
            <div className="flex flex-1 items-center justify-center">
              <p className="text-sm text-[#94A3B8]">Оберіть заявку зі списку</p>
            </div>
          )}
        </div>
      </div>
